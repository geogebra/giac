...
#define CAT_CATEGORY_3D 21
#define CAT_CATEGORY_LOGO 22 // should be the last one
#define XCAS_ONLY 0x80000000
  ...
  const catalogFunc completeCatfr[] = { // list of all functions (including some not in any category)
    ...
    {"line(A,B)", 0, "Droite donnee par equation ou 2 points", "y=x-1", "[0,0,0],[1,-2,3]", CAT_CATEGORY_3D | XCAS_ONLY},
...
const catalogFunc completeCaten[] = { // list of all functions (including some not in any category)
  ...
  {"line(equation)", 0, "Line of equation", "y=2x+1", "[0,0,0],[1,-2,3]", CAT_CATEGORY_PROGCMD},
...
namespace xcas {
  ...
  void update12(bool & found,bool &found2,
		double x1,double x2,double x3,double y1,double y2,double y3,double z1,double z2,double z3,int upcolor,int downcolor,int downupcolor,int downdowncolor,
  ...
  bool Graph2d::glsurface(const gen & surfaceg,int w,int h,int lcdz,GIAC_CONTEXT,
	...
	if (!ckmatrix(h,true))
	  return false;
	hypv.push_back(h._VECTptr->begin());
	hypv.push_back(h._VECTptr->end());
	hypcolor.push_back(int4(upcolor,downcolor,downupcolor,downdowncolor));
      }
    }
	  ...
	  for (int k=0;k<hypv.size();k+=2){
		...
		if (X>=window_xmin && X<=window_xmax && Y>=window_ymin && Y<=window_ymax && Z>=window_zmin && Z<=window_zmax && inside(x1,x2,x3,y1,y2,y3,x,y)){
		  update12(found,found2,
			   x1,x2,x3,y1,y2,y3,z1,z2,z3,u,d,du,dd,
	...
	for (int k=0;k<sphere_centerv.size();++k){
	  ...
	  if (X>=window_xmin && X<=window_xmax && Y>=window_ymin && Y<=window_ymax && Z>=window_zmin && Z<=window_zmax){
	    ...
	    update12(found,found2,
		     x-w2,x,x,y,y,y-w2,z+w0,z,z+w1,color.u,color.d,color.du,color.dd,
		     curx1,curx2,curx3,cury1,cury2,cury3,curz1,curz2,curz3,
		     cur2x1,cur2x2,cur2x3,cur2y1,cur2y2,cur2y3,cur2z1,cur2z2,cur2z3,
		     upcolor,downcolor,downupcolor,downdowncolor);
	  }
	...
	if (std::abs(v.y-v.x)<1e-4*(std::abs(v.y)+std::abs(v.x))){
#if 0
	  double X=m.x-xc,Y=m.y-yc,XY=Y-X;
	  double I=XY/xscale/2;
	  if (I-i>0.5 || I-i<=-0.5)
	    continue;
	  double3 M,N;
	  do_transform(invtransform,m.x,m.y.m.z,M.x,M.y,M.z);
#else
  ...
  void Graph2d::update_rotation(){
    ...
    plan_color.clear();sphere_color.clear();polyedre_color.clear();line_color.clear();curve_color.clear();
	...
	if (a.type==_VECT && b.type==_VECT && a._VECTptr->size()==3 && b._VECTptr->size()==3){
	  ...
	  if (A[0].type==_DOUBLE_ && A[1].type==_DOUBLE_ && A[2].type==_DOUBLE_ && B[0].type==_DOUBLE_ && B[1].type==_DOUBLE_ && B[2].type==_DOUBLE_ ){
	    ...
	    do_transform(transform,A[0]._DOUBLE_val,A[1]._DOUBLE_val,A[2]._DOUBLE_val,X,Y,Z);
	    double3 M(X,Y,Z);
	    do_transform(transform,B[0]._DOUBLE_val,B[1]._DOUBLE_val,B[2]._DOUBLE_val,X,Y,Z);
	    double3 N(X,Y,Z);
	    double3 v(N.x-M.x,N.y-M.y,N.z-M.z);
  ...
  void Graph2d::draw(){
    if (is3d){
      glsurface(grot,precision,precision,lcdz,contextptr,default_upcolor,default_downcolor,default_downupcolor,default_downdowncolor);
      if (show_axes){
	...
	xyz2ij(H,Hi,Hj);
	// X
	drawLine(Ai,Aj,Ci,Cj,COLOR_RED);
	drawLine(Bi,Bj,Di,Dj,COLOR_RED);
	drawLine(Ei,Ej,Gi,Gj,COLOR_RED);
	drawLine(Fi,Fj,Hi,Hj,COLOR_RED);
	// Y
	drawLine(Ai,Aj,Ei,Ej,COLOR_GREEN);
	drawLine(Bi,Bj,Fi,Fj,COLOR_GREEN);
	drawLine(Ci,Cj,Gi,Gj,COLOR_GREEN);
	drawLine(Di,Dj,Hi,Hj,COLOR_GREEN);
	// Z
	drawLine(Ai,Aj,Bi,Bj,12345);
	drawLine(Ci,Cj,Di,Dj,12345);
	drawLine(Ei,Ej,Fi,Fj,12345);
	drawLine(Gi,Gj,Hi,Hj,12345);
	// planes
	vecteur v(gen2vecteur(grot)),attrv(gen2vecteur(g));
	for (int i=0;i<v.size();++i){
	  gen cur=remove_at_pnt(v[i]);
	  int upcolor=44444;
