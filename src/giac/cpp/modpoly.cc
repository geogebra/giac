...
namespace giac {
  ...
  */
  ...
  int find_w(vector<int> & Wp,int shift,int p){
    int n=1<<shift,w=0;
...
#define EGCD_INT 1
  ...
  */
  ...
  // resultant of P and Q modulo m, modifies P and Q, 
  int resultant_int(vector<int> & P,vector<int> & Q,vector<int> & tmp1,vector<int> & tmp2,int m){
    if (P.size()<Q.size()){
      int res=(P.size() % 2==1 || Q.size() % 2==1)?1:-1; // (-1)^deg(P)*deg(Q)
      return res*resultant_int(Q,P,tmp1,tmp2,m);
    }
    if (P.size()==Q.size()){
      int coeff=Q[0];
      int invcoeff=invmod(coeff,m);
      ...
      return smod(res*resultant_int(Q,tmp2,P,tmp1,m),m);
    }
    // now P.size()>Q.size()
    if (Q.size()>=HGCD){
      if (debug_infolevel>2)
	...
	b.swap(P); 
      ...
      degv.push_back(Q.size()-1);
      int res=resultant_int(P,Q,tmp1,tmp2,m);
  ...
  gen mod_resultant(const modpoly & P,const modpoly & Q,double eps){
    ...
    res=resultant_int(p,q,tmp1,tmp2,m);
#if 0
    ntlresultant(P,Q,m,R,false); 
    if ((R-res)%m!=0)
      CERR << "bug\n";
    ...
    for (;h>sizeinbase2(pim) && proba<probamax;++niter){
      ...
      vecteur2vector_int(Q,m,q);
      int r;
      r=resultant_int(p,q,tmp1,tmp2,m); 
#if 0
      ntlresultant(P,Q,m,R,false);
      if ((R-r)%m!=0)
	CERR << "bug\n";
  ...
  */
  ...
  static void fft2pnopermafter( int *A, int n, int *W,int p,double invp,int step) {  
    ...
    for (;Acur!=An2;){
      ...
      B4.store(Acur);
      C4 = extend( A4+p-An4);
      Vec4uq W4((unsigned) Wcur[0],(unsigned) Wcur[step],(unsigned) Wcur[2*step],(unsigned) Wcur[3*step]),
	W4inv((unsigned) Wcur[n2],(unsigned) Wcur[n2+step],(unsigned) Wcur[n2+2*step],(unsigned) Wcur[n2+3*step]);
      C4 = C4*W4-((C4*W4inv)>>32)*p;
      // C4 += ( (Vec4q) C4>>63) & p;
      // An4=compress(C4);
      An4=compress(*(Vec4q*) &C4);
      An4 += ( (Vec4i) An4>>31)&p; 
      An4.store(An2cur);
      Acur+=4; An2cur+=4; Wcur+=4*step; continue;
#endif // VECTORCLASS_H
      fft_loop_p_(Acur,An2cur,Wcur,n2,p,invp);
      ++Acur;++An2cur; Wcur +=step;
      fft_loop_p_(Acur,An2cur,Wcur,n2,p,invp);
